...
export class ProcessManager {
    ...
    async kill() {
        try {
            ...
            // Wait for processes to exit and for ports to be freed
            const waitTimeoutSec = 15
            let waited = 0
            while (waited < waitTimeoutSec) {
                // If no RESTHeart is running anymore, break
            ...
            // If still running after timeout, attempt SIGKILL on remaining java/restheart processes
            if (await this.isRunning()) {
                logger.warning('Processes did not exit after SIGTERM, sending SIGKILL')
                const remaining = (await psList()).filter(
                    (p) => p.name === 'java' && p.cmd.includes('restheart')
                )
                for (const proc of remaining) {
                    try {
                        process.kill(proc.pid, 'SIGKILL')
                        logger.info(`Process ${proc.pid} force-killed\n`)
                    } catch (err) {
                        logger.error(`Failed to SIGKILL process ${proc.pid}: ${err.message}\n`)
                    }
                }
            }
        } catch (error) {
