...
import { addCommandExamples, commandDescriptions, getVersion } from './help.js'
import { logger, LogLevel } from './logger.js'
import { RESTHeartManager } from './restheart.js'
...
export function initCLI() {
    ...
    logger.log('\n')
    logger.log(chalk.green(' ========================='))
    logger.log(`   RESTHeart CLI v${getVersion()}`)
    logger.log(chalk.green(' =========================\n'))
    ...
    yargs(hideBin(process.argv))
            ...
            (yargs) => {
                yargs
                    .positional('restheart-version', {
                        describe: 'RESTHeart version to install (e.g., "latest", "8.10.1")',
                        type: 'string',
                        default: 'latest',
                    })
                    .option('force', {
            ...
            (yargs) => {
                yargs
                    .option('build', {
                        ...
                        type: 'boolean',
                        description:
                            'Build and deploy the plugin before starting the watch process',
                    })
                    .option('port', {
                        alias: 'p',
                        type: 'number',
                    ...
                    .option('debounce-time', {
                        type: 'number',
                        description:
                            'Time in milliseconds to wait after the last file change before rebuilding (default: 1000)',
                        default: 1000,
                    })
        ...
        .option('build-system', {
            ...
            type: 'string',
            description:
                'Preferred build system for plugin build/deploy (auto: detect from project files)',
            default: 'auto',
        })
        .help('h')
        .alias('h', 'help')
        ...
        .middleware([
            (argv) => {
                ...
                if (argv.buildSystem || argv['build-system']) {
                    rh.setBuildSystem(argv.buildSystem || argv['build-system'])
                }
            },
        ])
        .parse()
 ...
 */
export async function runCommand(command, argv, rh) {
    try {
        // Extract options
