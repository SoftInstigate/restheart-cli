...
export class RESTHeartManager {
    ...
    printConfiguration() {
        msg(chalk.cyan('repoDir: ') + this.repoDir)
        msg(chalk.cyan('cacheDir: ') + this.cacheDir)
        msg(chalk.cyan('rhDir: ') + this.rhDir)
        msg(chalk.cyan('httpPort: ') + this.httpPort)
        msg(chalk.cyan('debugMode: ') + this.debugMode)
        msg('\n')
    }
     ...
     */
    async killProcess(processName = 'restheart') {
        const processes = await psList()
        const filteredProcesses = processes.filter(
            (proc) => proc.name === 'java' && proc.cmd.includes(processName)
        )
        if (filteredProcesses.length === 0) {
            msg(chalk.cyan(`Process ${processName} not found`))
            return
        }
        console.log('Processes found:', filteredProcesses)
        filteredProcesses.forEach((proc) => {
            try {
                process.kill(proc.pid, 'SIGTERM')
                msg(chalk.cyan(`Process ${proc.pid} killed`))
            } catch (err) {
    ...
    async status() {
        if (await this.isRunning()) {
            msg(chalk.green('RESTHeart is running at localhost:' + this.httpPort))
        } else {
            msg(chalk.yellow('RESTHeart is not running at localhost:' + this.httpPort))
        }
    }
    ...
    async checkAndKill() {
        if (await this.isRunning()) {
            ...
            msg(chalk.yellow('RESTHeart is not running: nothing to kill'))
        }
    }
     ...
     * Install RESTHeart
     *
     * @param {string} restheartVersion
     * @param {boolean} forceInstall
     */
    install(restheartVersion, forceInstall) {
        ...
        msg(chalk.yellow(`Installing RESTHeart version "${restheartVersion}"...`))
        ...
        if (!fs.existsSync(this.rhDir)) {
            ...
            msg(
                chalk.cyan('Already installed: ') +
                    shell.exec(`java -jar ${path.join(this.rhDir, 'restheart.jar')} -v`, {
                        silent: true,
                    }).stdout
            )
            msg(chalk.cyan('Use the -f option to force a reinstall.'))
        }
    }
    ...
    async run(restheartOptions) {
        ...
        if (nohup === '') {
            msg(chalk.yellow('Warning: "nohup" not found.'))
        }
...
export { RESTHeartManager as default, msg }
