...
import { commandExists, createSpinner, ensureDir } from './utils.js'
...
export class Installer {
    ...
    /**
     * Install RESTHeart
     * @param {string} restheartVersion The version to install, or path to local core/target directory
    ...
    install(restheartVersion = 'latest', forceInstall = false) {
        try {
            ...
            // Resolve install strategy once: local path vs remote version
            const isLocalPath = restheartVersion.includes('/') || restheartVersion.includes('\\')
            const doInstall = isLocalPath
                ? () => this.installFromLocal(restheartVersion)
                : () => this.downloadRESTHeart(restheartVersion)

            logger.status(
                isLocalPath
                    ? `Installing RESTHeart from local build at "${restheartVersion}"...`
                    : `Installing RESTHeart version "${restheartVersion}"...`
            )
            ...
            if (fs.existsSync(rhDir)) {
                try {
                    ...
                    if (!fs.existsSync(jarPath)) {
                        logger.info('Will install a fresh copy...')
                        doInstall()
                        return
                    }
    ...
    /**
     * Install RESTHeart from local core/target directory
     * @param {string} localPath Path to core/target directory
     */
    installFromLocal(localPath) {
        try {
            ...
            this.validateLocalBuild(absolutePath)

            // Path to the tarball
            const tarballPath = path.join(absolutePath, 'restheart.tar.gz')

            logger.info('Extracting RESTHeart from local build...')

            // Extract the tarball to cache directory
            const extractResult = shell.exec(`tar -xzf ${tarballPath} -C ${cacheDir}`, {
                silent: true,
            })
            ...
            const jarPath = path.join(rhDir, 'restheart.jar')
            const verifyResult = shell.exec(`java -jar ${jarPath} -v`, { silent: true })
            ...
            logger.success('RESTHeart successfully installed from local build')
            logger.info('Installed from: ' + verifyResult.stdout.trim())
            process.exit(0)
        } catch (error) {
            ErrorHandler.processError(`Local installation failed: ${error.message}`, {
                exitProcess: true,
                showStack: true,
            })
        }
    ...
    /**
     * Validate local RESTHeart build directory
     * @param {string} localPath Path to core/target directory
     */
    validateLocalBuild(localPath) {
        // Check directory exists
        if (!fs.existsSync(localPath)) {
            throw new Error(`Directory does not exist: ${localPath}`)
        }

        // Check for restheart.tar.gz
        const tarballPath = path.join(localPath, 'restheart.tar.gz')
        if (!fs.existsSync(tarballPath)) {
            throw new Error(
                `restheart.tar.gz not found at ${localPath}. ` +
                    'Please run "mvn clean package" in the RESTHeart core directory first.'
            )
        }

        // Optional: Check for other expected files
        const jarPath = path.join(localPath, 'restheart.jar')
        if (!fs.existsSync(jarPath)) {
            logger.warning(`restheart.jar not found at ${localPath} (expected for complete build)`)
        }

        const libPath = path.join(localPath, 'lib')
        if (!fs.existsSync(libPath)) {
            logger.warning(`lib/ directory not found at ${localPath} (expected for complete build)`)
        }

        logger.debug(`Validated local build at ${localPath}`)
    }
    ...
    downloadAndExtractRESTHeart(url, file) {
        ...
        request.on('error', (err) => {
            try {
                ...
                logger.error(`Error removing incomplete file: ${error_.message}`)
            }
