...
export class ConfigManager {
    ...
    constructor(options = {}) {
        try {
            ...
            this.config = {
                ...
                buildSystem: options.buildSystem || 'auto',
            }
    ...
    validateConfig() {
        ...
        // Validate httpPort
        if (
            Number.isNaN(this.config.httpPort) ||
            this.config.httpPort < 1 ||
            this.config.httpPort > 65535
        ) {
        ...
        if (typeof this.config.debugMode !== 'boolean') {
            throw new TypeError(`Invalid debug mode: ${this.config.debugMode}`)
        }

        // Validate buildSystem
        const validBuildSystems = ['auto', 'maven', 'gradle']
        if (!validBuildSystems.includes(this.config.buildSystem)) {
            throw new Error(`Invalid build system: ${this.config.buildSystem}`)
        }
    }
    ...
    set(key, value) {
        ...
        if (key === 'buildSystem') {
            const validBuildSystems = ['auto', 'maven', 'gradle']
            if (!validBuildSystems.includes(value)) {
                ErrorHandler.configError(`Invalid build system: ${value}`, {
                    exitProcess: false,
                    showStack: false,
                })
                return
