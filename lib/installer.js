...
export class Installer {
        ...
        try {
            ...
            if (forceInstall) {
                ...
                try {
                    ...
                    ErrorHandler.fileSystemError(`Failed to clean cache directory: ${error.message}`, {
                        exitProcess: false,
                        showStack: false
                    })
                }
            ...
            if (!fs.existsSync(rhDir)) {
                ...
                try {
                    ...
                    if (!fs.existsSync(jarPath)) {
                        ErrorHandler.fileSystemError(`RESTHeart jar not found at ${jarPath}`, {
                            exitProcess: false,
                            showStack: false
                        })
                    ...
                    ErrorHandler.processError(`Failed to check existing installation: ${error.message}`, {
                        exitProcess: false,
                        showStack: false
                    })
                    logger.info('Will download a fresh copy...')
    ...
    downloadRESTHeart(restheartVersion) {
        try {
            ...
            logger.info(`Downloading RESTHeart ${restheartVersion} from ${url}`)
        ...
        file.on('finish', () => {
            file.close(() => {
                try {
                    ...
                    // Extract the tarball
                    const extractResult = shell.exec(`tar -xzf ${cacheDir}/restheart.tar.gz -C ${cacheDir}`, { silent: true })
                    ...
                    const verifyResult = shell.exec(`java -jar ${path.join(rhDir, 'restheart.jar')} -v`, { silent: true })
                    ...
                    if (this.builder) {
                        logger.info('Building and deploying plugins...')
                        this.builder.build()
                        this.builder.deploy()
                    }
