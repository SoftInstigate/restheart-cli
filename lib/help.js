...
export const commandDescriptions = {
    install: {
        desc: 'Install or update RESTHeart to a specific version or from local build',
        details:
            'Downloads and installs RESTHeart from GitHub releases, or installs from a local core/target directory. Supports specific versions or "latest".',
        examples: [
            ['$0 install', 'Install latest version'],
            ['$0 install 9.4.0', 'Install specific version'],
            ['$0 install ~/restheart/core/target', 'Install from local build'],
            ['$0 install ../restheart/core/target --force', 'Force install from local build'],
        ],
    },
    build: {
        desc: 'Build and deploy the RESTHeart plugin from the current directory',
        details:
            'Builds the project using the selected build system (auto-detected Maven or Gradle) and copies the resulting JARs to the plugins directory.',
        examples: [
            ['$0 build', 'Build and deploy plugin (auto-detected Maven or Gradle)'],
            ['$0 build --build-system gradle', 'Build and deploy using Gradle'],
            ['$0 build --build-system maven', 'Build and deploy using Maven'],
        ],
    },
    run: {
        desc: 'Start or restart RESTHeart with optional configuration',
        details:
            'Launches RESTHeart in the background with the specified options. Supports debugging via JDWP.',
        examples: [
            ['$0 run', 'Run with default settings'],
            ['$0 run --build', 'Build before running'],
            ['$0 run --build --build-system gradle', 'Build with Gradle before running'],
    ...
    kill: {
        ...
        details:
            'Terminates all running RESTHeart processes. Can target a specific port if specified.',
        examples: [
    ...
    watch: {
        ...
        details:
            'Monitors source files for changes and automatically rebuilds/redeploys/restarts RESTHeart.',
        examples: [
            ['$0 watch', 'Watch with default settings'],
            ['$0 watch --build', 'Build before watching'],
            ['$0 watch --build --build-system maven', 'Build with Maven before watching'],
