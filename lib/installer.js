...
export class Installer {
    ...
    install(restheartVersion = 'latest', forceInstall = false) {
        try {
            ...
            // Check if restheartVersion is a path to local core/target
            const isLocalPath = restheartVersion.includes('/') || restheartVersion.includes('\\')

            if (isLocalPath) {
                logger.warning(`Installing RESTHeart from local build at "${restheartVersion}"...`)
            } else {
    ...
    installFromLocal(localPath) {
        try {
            ...
            const extractResult = shell.exec(
                `tar -xzf ${tarballPath} -C ${cacheDir}`,
                { silent: true }
            )
    ...
    validateLocalBuild(localPath) {
        ...
        if (!fs.existsSync(jarPath)) {
            logger.warning(
                `restheart.jar not found at ${localPath} (expected for complete build)`
            )
        }
        ...
        if (!fs.existsSync(libPath)) {
            logger.warning(
                `lib/ directory not found at ${localPath} (expected for complete build)`
            )
        }
