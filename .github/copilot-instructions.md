...
-   **Subsystems**:
    ...
    -   `lib/watcher.js`: Watches for changes in: - Java sources: `src/main/**/*.java` - RESTHeart config files passed via `-o` (e.g., `etc/localhost.yml`) - Maven POM files: any `pom.xml` in the project (single or multi-module)
