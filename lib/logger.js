...
export class Logger {
     ...
     */
    constructor(options = {}) {
        this.level = options.level === undefined ? LogLevel.INFO : options.level
    ...
    /**
     * Log an operational status message (key lifecycle events: starting, stopping, watching).
     * Shown at INFO level, bold white to distinguish from informational detail.
     * @param {string} message Message to log
     */
    status(message) {
        if (this.level <= LogLevel.INFO) {
            this.outputFn(this.formatMessage(chalk.bold(message)))
        }
    }

    /**
     * Log a plain message (always shown, bypasses level filtering)
     * @param {string} message Message to log
     */
    log(message) {
