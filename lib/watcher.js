...
export class Watcher {
    ...
    watchFiles(restheartOptions = '', watchOptions = {}) {
        try {
            ...
            // Always watch build-system files (single or multi-module projects)
            const pomGlob = path.join(repoDir, '**/pom.xml')
            const gradleGlob = path.join(repoDir, '**/build.gradle')
            const gradleKtsGlob = path.join(repoDir, '**/build.gradle.kts')
            const settingsGradleGlob = path.join(repoDir, '**/settings.gradle')
            const settingsGradleKtsGlob = path.join(repoDir, '**/settings.gradle.kts')

            // Setup watch options with defaults
            const paths = watchOptions.paths || [
                ...defaultPaths,
                pomGlob,
                gradleGlob,
                gradleKtsGlob,
                settingsGradleGlob,
                settingsGradleKtsGlob,
                ...configFiles,
            ]
    ...
    async processFileUpdate(filePath, restheartOptions, configFiles = []) {
        ...
        try {
            ...
            const isConfigChange =
                isConfigExplicit || (isYamlOrProperties && !isJavaChange && !isBuildConfigChange)
            ...
            if (isJavaChange || isBuildConfigChange) {
                // Full rebuild path
...
function validateWatchPath(watchPath) {
    if (
        ...
        watchPath.endsWith('build.gradle') ||
        watchPath.endsWith('build.gradle.kts') ||
        watchPath.endsWith('settings.gradle') ||
        watchPath.endsWith('settings.gradle.kts') ||
