...
export class RESTHeartManager {
    ...
    printConfiguration() {
        ...
        logger.log(chalk.cyan('repoDir: ') + config.repoDir)
        logger.log(chalk.cyan('cacheDir: ') + config.cacheDir)
        logger.log(chalk.cyan('rhDir: ') + config.rhDir)
        logger.log(chalk.cyan('httpPort: ') + config.httpPort)
        logger.log(chalk.cyan('debugMode: ') + config.debugMode)
        logger.log(chalk.cyan('buildSystem: ') + config.buildSystem)
        logger.log('\n')
    }
     ...
     * @param {string} buildSystem Build system preference (auto|maven|gradle)
     */
    setBuildSystem(buildSystem) {
        this.configManager.set('buildSystem', buildSystem)
    }
