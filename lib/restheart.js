...
export class RESTHeart {
    ...
    async kill() {
        msg(`Killing RESTHeart at localhost:${this.httpPort}`, chalk.yellow)
    ...
    install(restheartVersion, forceInstall) {
        ...
        if (!fs.existsSync(this.rhDir)) {
            ...
            msg('RESTHeart already installed. Use the -f option to force a reinstall.', chalk.reset)
        }
    }
    ...
    build(mvnParams = '') {
        ...
        if (!fs.existsSync(path.join(this.repoDir, 'mvnw'))) {
            ...
            this.commandExists('mvn')
            mvnCommand = `mvn -f pom.xml ${mvnParams}`
        } else {
            this.commandExists('./mvnw')
        }
    ...
    async run(restheartOptions) {
        this.commandExists('java')
        if (!(await this.isRunning())) {
            if (onlyPrintConfig(restheartOptions)) {
                msg('Printing RESTHeart configuration', chalk.yellow)
                shell.exec(
                    `java -jar ${path.join(this.rhDir, 'restheart.jar')} ${restheartOptions}`
                )
                return
            }

            const command = `nohup java -jar ${path.join(this.rhDir, 'restheart.jar')} ${restheartOptions} &> ${path.join(this.repoDir, 'restheart.log')} &`

            msg(`Starting RESTHeart with options: ${restheartOptions}`, chalk.yellow)
    ...
    watchFiles(restheartOptions) {
        ...
        watcher.on('change', async (filePath) => {
            ...
            debounceTimeoutId = setTimeout(async () => {
                ...
                spinner.succeed(`File changed: ${filePath}`)
