...
export class Watcher {
        ...
        try {
            ...
            if (typeof restheartOptions === 'string' && restheartOptions.trim().length > 0) {
                ...
                while ((m = regex.exec(restheartOptions)) !== null) {
                    if (m[1]) {
                        // Normalize relative paths to repoDir
            ...
            // remember resolved config files for use in the change handler
            this._watchedConfigFiles = configFiles
            ...
            watcher.on('change', async (filePath) => {
                ...
                debounceTimeoutId = setTimeout(async () => {
                    ...
                    try {
                        await this.processFileUpdate(filePath, restheartOptions)
                    } catch (error) {
    ...
    async processFileUpdate(filePath, restheartOptions) {
        ...
        try {
            ...
            const isJavaChange = absPath.endsWith('.java') && absPath.includes(path.join(repoDir, 'src', 'main'))
            const isPomChange = path.basename(absPath).toLowerCase() === 'pom.xml'

            const configFiles = this._watchedConfigFiles || []
            const isConfigExplicit = configFiles.some((cfg) => path.resolve(cfg) === absPath)
            const isYamlOrProperties = ['.yml', '.yaml', '.properties'].some((ext) => absPath.endsWith(ext))
            const isConfigChange = isConfigExplicit || (isYamlOrProperties && !isJavaChange && !isPomChange)
...
function validateWatchPath(watchPath) {
    if (watchPath.endsWith('pom.xml')
        || watchPath.endsWith('.yml')
        || watchPath.endsWith('.yaml')
        || watchPath.endsWith('.properties')) {
        // explicit file or specific filename glob
        // If it's a glob (contains **), check the base dir
